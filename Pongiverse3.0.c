...
int main(void) 
    ...
    Texture2D tennisBall = LoadTexture("ball.png");
    ...
    while (homeScreen && !WindowShouldClose()) 
    {
        ...
        // Define hitboxes for the three modes
...
// Clicking on homescreen using mousePosition
Rectangle hitbox1 = {330, 560, 200, 50}; // x, y, width, height for Mode 1
Rectangle hitbox2 = {595, 560, 200, 50}; // x, y, width, height for Mode 2
Rectangle hitbox3 = {865, 560, 200, 50}; // x, y, width, height for Mode 3


    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        // Check which hitbox is clicked
        if (CheckCollisionPointRec(mousePosition, hitbox1)) {
            currentMode = 1; // Mode 1 selected
            homeScreen = false; // Exit home screen
        } else if (CheckCollisionPointRec(mousePosition, hitbox2)) {
            currentMode = 2; // Mode 2 seleted
            homeScreen = false; // Exit home screen
        } else if (CheckCollisionPointRec(mousePosition, hitbox3)) {
            currentMode = 3; // Mode 3 selected
            homeScreen = false; // Exit home screen
        }
    }

    // Optional: Draw hitboxes for debugging
    DrawRectangleRec(hitbox1, Fade(RED, 0.3f)); // Semi-transparent red for Mode 1
    DrawRectangleRec(hitbox2, Fade(GREEN, 0.3f)); // Semi-transparent green for Mode 2
    DrawRectangleRec(hitbox3, Fade(BLUE, 0.3f)); // Semi-transparent blue for Mode 3
}
...
if (IsKeyPressed(KEY_B)) {
    pauseMenuActive = !pauseMenuActive; // Toggle pause menu state
}
// If the pause menu is active, show the quit screen menu
if (pauseMenuActive) {
    BeginDrawing();
    ClearBackground(BLACK); // Optional: Use a dimmed black background

    // Draw the quit screen image
    Texture2D quitScreenTexture = LoadTexture("quitscreen.png"); // Ensure this texture is loaded
    DrawTexture(quitScreenTexture, 0, 0, WHITE);


    // Check for input to select options
    if (IsKeyPressed(KEY_P)) // Press P to resume the game
    { 
        pauseMenuActive = false;
    } 
    else if (IsKeyPressed(KEY_R)) // Press R to reset to the main menu
    { 
        pauseMenuActive = false;
        homeScreen = true;   // Return to the home screen
        instructionsScreen = false; // Exit any instruction screen
        gameEnded = false;   // Reset game end state
        ballActive = true;   // Reactivate the ball
        score1 = 0;          
        score2 = 0;         
        ballPosition = (Vector2){(SCREEN_WIDTH / 2) - (BALL_SIZE / 2), (SCREEN_HEIGHT / 2) - (BALL_SIZE / 2)}; // Reset ball position
        ballVelocity = (Vector2){BALL_SPEED, BALL_SPEED}; // Reset ball velocity
        paddle1X = 0;        // Reset left paddle position
        paddle1Y = (SCREEN_HEIGHT - currentPaddle.height) / 2;
        paddle2X = SCREEN_WIDTH - currentPaddle.width - 90; // Reset right paddle position
        paddle2Y = (SCREEN_HEIGHT - currentPaddle.height) / 2;
    } 
    else if (IsKeyPressed(KEY_M)) // Press M to return to mode selection
    { 
        pauseMenuActive = false;
        homeScreen = true; // Set home screen flag to true
        instructionsScreen = false; // Exit instruction screen
        gameEnded = false; // Reset game end state
        score1 = 0; // Reset score for Player 1
        score2 = 0; // Reset score for Player 2
    }

    EndDrawing();
    continue; // Skip rendering the rest of the game if pause menu is active
}
